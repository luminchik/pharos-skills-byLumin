...
| Configure local power-user unlimited policy | `node scripts/pharos-policy.mjs --enable --power-user --signer <wallet>` | Only when the user explicitly asks for unlimited access |
...
node scripts/pharos-policy.mjs --enable --power-user --signer 0xYourWallet
```
...
- Use `--power-user` or `--unlimited` only when the user explicitly asks for unlimited mainnet access. Explain that scripts still require `--broadcast` and policy is local.
